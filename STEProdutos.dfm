object FSTEProdutos: TFSTEProdutos
  Left = 0
  Top = 0
  Caption = 'FSTEProdutos'
  ClientHeight = 468
  ClientWidth = 610
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OnActivate = FormActivate
  OnClose = FormClose
  OnResize = FormResize
  TextHeight = 16
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 610
    Height = 29
    Align = alTop
    Caption = 'Produtos cadastrados'
    Color = clBlue
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clYellow
    Font.Height = -16
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
  end
  object Panel2: TPanel
    Left = 0
    Top = 29
    Width = 610
    Height = 394
    Align = alClient
    TabOrder = 1
    object GridProds: TDBGrid
      Left = 1
      Top = 1
      Width = 608
      Height = 279
      Align = alClient
      DataSource = FSTEPrincipal.DSProdutos
      DrawingStyle = gdsClassic
      FixedColor = clMoneyGreen
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -13
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnDblClick = GridProdsDblClick
      Columns = <
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'Seq'
          ReadOnly = True
          Title.Alignment = taCenter
          Width = 53
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Descr'
          Title.Caption = 'Descri'#231#227'o'
          Width = 325
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Valor'
          Title.Alignment = taRightJustify
          Width = 101
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ZL_Tipo'
          Title.Caption = 'Tipo produto'
          Visible = True
        end>
    end
    object PanManut: TPanel
      Left = 1
      Top = 280
      Width = 608
      Height = 113
      Align = alBottom
      TabOrder = 1
      Visible = False
      DesignSize = (
        608
        113)
      object Label1: TLabel
        Left = 514
        Top = 4
        Width = 83
        Height = 16
        Alignment = taRightJustify
        Anchors = [akTop, akRight]
        Caption = 'Incluir produto'
      end
      object Label2: TLabel
        Left = 8
        Top = 26
        Width = 65
        Height = 16
        Alignment = taRightJustify
        Caption = 'Descri'#231#227'o:'
      end
      object Label3: TLabel
        Left = 38
        Top = 56
        Width = 35
        Height = 16
        Alignment = taRightJustify
        Caption = 'Valor:'
      end
      object Label4: TLabel
        Left = 42
        Top = 84
        Width = 31
        Height = 16
        Alignment = taRightJustify
        Caption = 'Tipo:'
      end
      object dbDescricao: TDBEdit
        Left = 76
        Top = 22
        Width = 522
        Height = 24
        Anchors = [akLeft, akTop, akRight]
        CharCase = ecUpperCase
        DataField = 'Descr'
        DataSource = FSTEPrincipal.DSProdutos
        TabOrder = 0
        OnKeyDown = dbDescricaoKeyDown
        OnKeyPress = dbDescricaoKeyPress
      end
      object dbValor: TDBEdit
        Left = 76
        Top = 52
        Width = 121
        Height = 24
        DataField = 'Valor'
        DataSource = FSTEPrincipal.DSProdutos
        TabOrder = 1
        OnKeyDown = dbValorKeyDown
        OnKeyPress = dbValorKeyPress
      end
      object btGravaProd: TBitBtn
        Left = 515
        Top = 60
        Width = 33
        Height = 29
        Hint = 'Gravar produto'
        Anchors = [akTop, akRight]
        Glyph.Data = {
          
          5990555555555555577755555555555555555555555555555555}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        OnClick = btGravaProdClick
      end
      object btCancelaProd: TBitBtn
        Left = 564
        Top = 60
        Width = 33
        Height = 29
        Hint = 'Cancelar inclus'#227'o'
        Anchors = [akTop, akRight]
        Glyph.Data = {
          
          6199666666666666678866666666666666666666666666666666}
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        TabOrder = 4
        OnClick = btCancelaProdClick
      end
      object dbLkTipo: TDBLookupComboBox
        Left = 76
        Top = 82
        Width = 145
        Height = 24
        DataField = 'Tipo'
        DataSource = FSTEPrincipal.DSProdutos
        KeyField = 'CodTp'
        ListField = 'DescrTp'
        ListSource = FSTEPrincipal.SDSTipos
        TabOrder = 2
        OnKeyDown = dbLkTipoKeyDown
        OnKeyPress = dbLkTipoKeyPress
      end
    end
  end
  object PanRodape: TPanel
    Left = 0
    Top = 423
    Width = 610
    Height = 45
    Align = alBottom
    TabOrder = 2
    DesignSize = (
      610
      45)
    object LabNRegs: TLabel
      Left = 164
      Top = 12
      Width = 66
      Height = 16
      Caption = 'LabNRegs'
    end
    object btSair: TBitBtn
      Left = 565
      Top = 5
      Width = 33
      Height = 29
      Hint = 'Sair'
      Anchors = [akTop, akRight]
      Glyph.Data = {
        
        898989898989898989892E36444448412D1C8989898989898989}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = btSairClick
    end
    object btIncluir: TBitBtn
      Left = 409
      Top = 6
      Width = 33
      Height = 29
      Hint = 'Incluir produto'
      Anchors = [akTop, akRight]
      Glyph.Data = {
        
        0808080808080808080808080808080808080808080808080808}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = btIncluirClick
    end
    object btExcluir: TBitBtn
      Left = 459
      Top = 5
      Width = 33
      Height = 29
      Hint = 'Excluir produto'
      Anchors = [akTop, akRight]
      Glyph.Data = {
        
        0606060606060606060606060606060606060606060606060606}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = btExcluirClick
    end
    object btImprimir: TBitBtn
      Left = 509
      Top = 5
      Width = 33
      Height = 29
      Hint = 'Imprimir '
      Anchors = [akTop, akRight]
      Glyph.Data = {
        
        5353535353532333322C2C24241E255353535353535353535353}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      Visible = False
      OnClick = btImprimirClick
    end
    object NavProds: TDBNavigator
      Left = 8
      Top = 6
      Width = 150
      Height = 29
      DataSource = FSTEPrincipal.DSProdutos
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbRefresh]
      Hints.Strings = (
        'Primeiro'
        'Anterior'
        'Pr'#243'ximo'
        #218'ltimo'
        'Insert record'
        'Delete record'
        'Edit record'
        'Post edit'
        'Cancel edit'
        'Atualizar'
        'Apply updates'
        'Cancel updates')
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
    end
    object DBRadioGroup1: TDBRadioGroup
      Left = 212
      Top = -28
      Width = 185
      Height = 105
      Caption = 'DBRadioGroup1'
      TabOrder = 5
    end
  end
end
